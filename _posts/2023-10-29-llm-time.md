...
- **Continuous likelihoods** Modeling sequences of individual digits has additional benefits beyond good samples.
  ...
  - finally, to obtain the likelihood $\log p(z)$ in the original input space, add a change of variables factor $\log |\frac{dx}{dz}|$, where $z \rightarrow x = s(z)$ is the resacling operation in the pre-processing.
